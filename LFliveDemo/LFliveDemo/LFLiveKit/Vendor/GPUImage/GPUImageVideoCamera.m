...
- (void)processVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer;
{
    if (capturePaused)
    {
        return;//如果处在暂停状态直接不处理
    }
    
    CFAbsoluteTime startTime = CFAbsoluteTimeGetCurrent(); //获取当前时间 CFAbsoluteTime 其实就是double
    CVImageBufferRef cameraFrame = CMSampleBufferGetImageBuffer(sampleBuffer); //获取一个CVImageBufferRef
    int bufferWidth = (int) CVPixelBufferGetWidth(cameraFrame); //获取宽度
    int bufferHeight = (int) CVPixelBufferGetHeight(cameraFrame); //获取高度
    CFTypeRef colorAttachments = CVBufferGetAttachment(cameraFrame, kCVImageBufferYCbCrMatrixKey, NULL);//获取色彩空间
	...
	CMTime currentTime = CMSampleBufferGetPresentationTimeStamp(sampleBuffer);//获取时间点
